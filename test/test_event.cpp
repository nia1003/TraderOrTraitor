...
int main() {
    ...
    // 回合二 事件2：遠端工作需求激增
    events.emplace_back(
        unordered_map<string, double>{{"MSFT", 1.5}, {"AAPL", 1.2}},
    ...
    // 回合二 事件3：消費者囤積必需品
    events.emplace_back(
        unordered_map<string, double>{{"COST", 1.7}, {"KO", 1.3}},
    ...
    // 回合三 事件1: 科技公司財報超預期
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.8}, {"MSFT", 1.6}},
    ...
    // 回合三 事件2: 疫苗研發取得進展
    events.emplace_back(
        unordered_map<string, double>{{"MRNA", 2.0}, {"UBER", 1.2}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"TSM", 0.7}, {"INTC", 0.8}},
    ...
    // 回合四 事件1: 科技公司推出新產品線
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.8}, {"MSFT", 1.5}},
    ...
    // 回合四 事件2: 疫苗研發進入最後階段
    events.emplace_back(
        unordered_map<string, double>{{"MRNA", 2.0}, {"UBER", 1.3}, {"UAL", 1.4}, {"DAL", 1.4}},
    ...
    // 回合四 事件3: 消費必需品需求增長
    events.emplace_back(
        unordered_map<string, double>{{"COST", 1.6}, {"KO", 1.3}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.8}, {"INTC", 0.7}},
        "蘋果公司（Apple Inc.）在 11 月發佈首款搭載自研 M1 晶片的 Mac 電腦，標誌著從英特爾（Intel）處理器過渡的開始，提升產品性能和能效",
        10,  // AAPL 積極影響；INTC 負面影響
        "Tech Industry Reports"
    );

    // 回合五 事件2: 半導體產業供應鏈緊張
    events.emplace_back(
        unordered_map<string, double>{{"TSM", 1.5}, {"MSFT", 0.8}, {"AAPL", 0.9}},
    ...
    // 回合五 事件3: 全球股市強勁反彈
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.2}, {"MSFT", 1.2}, {"UBER", 1.2}, {"MRNA", 1.2}, {"COST", 1.2}, {"KO", 1.2}, {"TSM", 1.2}, {"INTC", 1.2}, {"UAL", 1.2}, {"DAL", 1.2}}, // "ALL" 代表全體股票
    ...
    events.emplace_back(
        unordered_map<string, double>{{"MSFT", 1.2}, {"AAPL", 1.1}, {"DAL", 0.8}, {"UBER", 0.7}},
    ...
    // 回合六 事件2: 疫苗接種計劃進展迅速
    events.emplace_back(
        unordered_map<string, double>{{"MRNA", 1.8}, {"UAL", 1.5}, {"DAL", 1.5}, {"COST", 1.0}, {"KO", 1.0}},
    ...
    // 回合六 事件3: 虛擬貨幣市場崩盤
    events.emplace_back(
        unordered_map<string, double>{{"MSFT", 0.9}, {"TSM", 0.8}, {"AAPL", 1.1}, {"COST", 1.2}},
    ...
    // 回合七 事件1: 台積電宣布全球擴張計劃
    events.emplace_back(
        unordered_map<string, double>{{"TSM", 1.8}, {"INTC", 0.7}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 0.8}, {"MSFT", 0.9}, {"COST", 0.7}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.7}, {"KO", 0.6}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 1.7}, {"TSM", 0.9}, {"MSFT", 0.8}},
    ...
    // 回合八 事件2: 英特爾製程技術突破
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 1.6}, {"TSM", 0.8}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"MRNA", 0.6}, {"UAL", 0.7}, {"DAL", 0.7}, {"COST", 1.3}, {"KO", 1.2}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 1.9}, {"TSM", 0.7}},
    ...
    events.emplace_back(
        ...
        "由於對疫苗安全性的質疑，全球疫苗接種率下降，導致 COVID-19 疫情反彈，各國重新實施封鎖措施，經濟復甦受阻",
        -10,  // 負面影響
        "Global Health Watch"
    );
    ...
    // 回合九 事件3: 科技行業持續領跑經濟
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1.6}, {"MSFT", 1.5}, {"TSM", 1.3}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 2.0}, {"TSM", 0.5}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"UAL", 1.7}, {"DAL", 1.6}},
