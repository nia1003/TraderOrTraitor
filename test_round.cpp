...
constexpr int EVENT_IMPACT_PARAMETER = 2;
...
int main() {
    ...
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", -1.0}, {"MSFT", -1.0}, {"UBER", -1.0}, {"MRNA", -0.8}, {"COST", -1.0}, {"KO", -1.0}, {"TSM", -1.0}, {"INTC", -1.0}, {"UAL", -1.0}, {"DAL", -1.0}}, // 使用 "ALL" 表示全體股票
    ...
    events.emplace_back(
        ...
        0.08 * EVENT_IMPACT_PARAMETER,  // 穩定正面影響
    ...
    events.emplace_back(
        ...
        0.12 * EVENT_IMPACT_PARAMETER,  // 正面影響
        "Tech News"
    );
    ...
    events.emplace_back(
        ...
        0.05 * EVENT_IMPACT_PARAMETER,  // 負面影響
        "Industry Reports"
    );
    ...
    events.emplace_back(
        ...
        0.08 * EVENT_IMPACT_PARAMETER,  // 正面影響
        "Tech Industry Reports"
    );
    ...
    events.emplace_back(
        ...
        0.1 * EVENT_IMPACT_PARAMETER,  // 強烈正面影響
    ...
    events.emplace_back(
        ...
        0.1 * EVENT_IMPACT_PARAMETER,  // AAPL 積極影響；INTC 負面影響
        "Tech Industry Reports"
    );
    ...
    events.emplace_back(
        ...
        0.09 * EVENT_IMPACT_PARAMETER,  // 供應鏈緊張對大部分企業是負面影響
        "Industry Reports"
    );

    // 回合五 事件3: 全球股市強勁反彈
    events.emplace_back(
        unordered_map<string, double>{{"AAPL", 1}, {"MSFT", 1}, {"UBER", 1}, {"MRNA", 1}, {"COST", 1}, {"KO", 1}, {"TSM", 1}, {"INTC", 1}, {"UAL", 1}, {"DAL", 1}}, // "ALL" 代表全體股票
        "受疫苗研發進展和經濟刺激政策影響，全球股市在 11 月出現強勁反彈，投資者信心回升",
        0.2 * EVENT_IMPACT_PARAMETER,  // 正面影響
        "Global Market News"
    );
    ...
    events.emplace_back(
        ...
        0.07 * EVENT_IMPACT_PARAMETER,  // 負面影響
        "Crypto Market News"
    );

    // 回合七 事件1: 台積電宣布全球擴張計劃
    events.emplace_back(
        unordered_map<string, double>{{"TSM", 1.0}, {"INTC", -0.5}},
        "台積電（TSMC）宣布未來三年內在全球建造六座新廠，擴大產能以應對晶片短缺問題",
        0.14 * EVENT_IMPACT_PARAMETER,  // 正面影響
        "Tech Industry Reports"
    );
    ...
    events.emplace_back(
        ...
        0.06 * EVENT_IMPACT_PARAMETER,  // 負面影響
    ...
    events.emplace_back(
        ...
        0.07 * EVENT_IMPACT_PARAMETER,  // 正面影響
        "Consumer Market Reports"
    );

    // 回合八 事件1: 英特爾宣布新建晶圓代工廠
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 0.9}, {"TSM", -0.4}, {"MSFT", -0.2}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"MRNA", -0.6}, {"UAL", -0.3}, {"DAL", -0.3}, {"COST", 0.8}, {"KO", 0.8}},
    ...
    events.emplace_back(
        unordered_map<string, double>{{"INTC", 1.0}, {"TSM", -0.8}},
        "收購格羅方德後，3奈米製程良率持續成長，英特爾（Intel）成功搶到大筆客戶訂單，超越台積電（TSMC），成為全球最大的晶圓代工廠，吸引大量客戶轉單",
        0.18 * EVENT_IMPACT_PARAMETER,  // 強烈正面影響
        "Tech Market Reports"
    );
    ...
    {
        {"AAPL", new Stock("AAPL", "Apple Inc.", "Technology", 150, "疫情期間持續推出創新產品，並加速研發自家晶片，硬體需求保持強勁。然而，疫情後需關注一般消費者的購買力是否能夠支撐其營收增長。", 0.8)},
        {"MSFT", new Stock("MSFT", "Microsoft", "Technology", 280, "受益於遠端辦公需求以及 Azure 雲端業務的增長，近期業績表現穩定，未來發展值得關注。", 0.8)},
        {"UBER", new Stock("UBER", "Uber", "Technology", 55, "疫情期間載客需求顯著減少，業務面臨挑戰，但未來需求回升的可能性仍需觀察。", 1.3)},
        {"MRNA", new Stock("MRNA", "Moderna", "Biotech", 150, "疫苗研發取得成功，疫情後期業務增長顯著，但長期表現仍存在不確定性，須注意疫苗是否能被各國認可。", 1.7)},
        {"COST", new Stock("COST", "Costco", "Consumer Staples", 520, "疫情期間囤貨需求激增，但未來趨勢仍需觀察", 0.6)},
        {"KO", new Stock("KO", "Coca-Cola", "Consumer Staples", 60, "飲料需求保持穩定，但長期增長可能取決於消費者習慣的變化", 0.7)},
        {"TSM", new Stock("TSM", "Taiwan Semiconductor Manufacturing", "Semiconductor", 120, "作為晶圓代工龍頭，持續受益於先進晶片需求，但面臨英特爾新競爭壓力", 1.0)},
        {"INTC", new Stock("INTC", "Intel", "Semiconductor", 55, "老牌半導體公司，近期擴張晶圓代工業務，與台積電處於劇烈競爭的狀態", 1.0)},
        {"UAL", new Stock("UAL", "United Airlines", "Airlines", 40, "旅遊需求驟降，業績遭受重創，但疫苗問世可能帶來復甦希望", 1.2)},
        {"DAL", new Stock("DAL", "Delta Air Lines", "Airlines", 45, "作為航空業龍頭之一，疫苗普及推動旅遊需求回升，但挑戰依然存在", 1.2)}
    };
